...
public class ChangeAddress extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String add = request.getParameter("newadd");
            try {
                Connection con = DataBaseConn.getConnection();
                PreparedStatement ps = con.prepareCall("update userinfo set user_address=? where user_id=?");
                ps.setString(1, add);
                ps.setString(2, client_id);
                int num = ps.executeUpdate();
                if (num > 0) {
                    response.sendRedirect("client/client_address.jsp");
                } else {
                    out.println("Some thing happne");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
