    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                PreparedStatement ps = con.prepareCall("insert into customer_support_client values (?,?,?,?,?)");
                ps.setString(1, date);
                ps.setString(2, time);
                ps.setString(3, s);
                ...
                if (done > 0) {
                    response.sendRedirect("support/customer_care.jsp");
                } else {
                    response.sendRedirect("support/customer_care.jsp?msg=fail");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
