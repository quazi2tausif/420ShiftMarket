    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                PreparedStatement ps = con.prepareCall("update product set product_cost = ? where product_id = ?");
                ps.setString(1, pro_cost);
                ps.setString(2, pro_id);
                int done = ps.executeUpdate();
                if (done > 0) {
                    response.sendRedirect("administrator/admin_option.jsp");
                } else {
                    response.sendRedirect("administrator/admin_option.jsp?msg=editpricefail");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
